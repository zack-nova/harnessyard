...
func TestHyardStartFailsClosedWithPromptFallbackForUnsupportedLaunchers(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Contains(t, stdout, "Harness Start cannot launch "+tc.framework+" interactively.")
			require.Contains(t, stdout, "launcher_status: unsupported")
			require.Contains(t, stdout, "launcher_detection_status: not_found")
			require.Contains(t, stdout, "terminal_cli_detected: false")
			require.Contains(t, stdout, "manual_next_action:")
			require.Contains(t, stdout, "hyard start --print-prompt")
			require.Contains(t, stdout, "usage:")
			require.Contains(t, stdout, "hyard start --dry-run --json")
			require.Contains(t, stdout, "Start Prompt")
			require.Contains(t, stdout, "First handle any pending Harness Runtime bootstrap work.")
			require.NotContains(t, stdout, "interactive_start: success")
